...
class neuron {
	...
	protected:
		...
		void tidsBasertDepolariseringAvNeuron_til_egenfyring(int antallKlokkeTikkSiden)
		{
			// Tabell for faktor for degradering, på grunnlag av tid.
				// #define DEGRADERINGSFAKTOR_FOR_TIDSINTEGRASJON_I_NEURON 0.96
				// #define DEF_MAKS_TID_MELLOM_OPPDATERINGER_FOR_KVART_NEURON 100 
		...
		virtual int sendInnPostsynaptiskEksitatoriskEllerInhibitoriskSignal( int nInnsignalArg )
		{
			if( ulTidsiterasjoner -  ulTimestampFyring > REFRACTION_TIME_NEURON ) return -9;
			...
			*/
			...
			if(ulTidsiterasjoner > ulTimestampForrigeOppdatering ){
				...
				while( ulTimestampForrigeOppdatering < ulTidsiterasjoner ){
					...
					//if(nVerdiForDepolarisering > TERSKEL_DEFAULT){ fyr(); break; }
					
					// XXX Her er bedre å lage en funk: "kor-mykje-får-eg-om-eg-hadde-oppdater-kvar-runde-i-så-manke-tiks(int)" --gir endring i depol
					tidsBasertDepolariseringAvNeuron_til_egenfyring(ulTidsiterasjoner-ulTimestampForrigeOppdatering);
			...
			unsigned uTidsDiffTilNesteFyring =  ( (TERSKEL_DEFAULT - nVerdiForDepolarisering) / nVerdiForDepolariseringEndringsrate );
		...
		static neuron* s_get_npNesteNeuron_med_oppdatering_neste_tid()
		{
			...
			while( ++iter != sNesteFyringForNeuron.end() ){
				if( (*iter).second <= (ulTidsiterasjoner +1) ){	
					...
					// 						bI_gang_med_soek_etter_neuron_med_fyring_neste_tid = true (kommer ikkje til kallet under)
					return (*iter).first;
				}
			}
